...
> | No cross-contamination of data between End Users (§8.7) | Each user registers their own AppKey and holds their own Keychain entry (`saxotrader-live` / `saxotrader-sim`). Tokens from one user's account cannot be accessed by another. |
>
...
> entering credentials (see the TLS verification command in [Authenticate](#3-authenticate) below), and (b) note that `developer.saxo` is listed
...
claude plugin install saxotrader@zaliori-io
```
...
| **Per-user isolation** | Each user registers their own AppKey and gets their own Keychain entry (`saxotrader-live` / `saxotrader-sim`). There is no shared credential or multi-tenant token store. |
...
├── README.md
├── LICENSE
├── reinstall.sh                  ← clean reinstall script (use instead of plugin update)
├── skills/
│   └── saxo/
...
│       │   └── test_regression.py   ← 28-test regression suite (sim or live)
│       └── references/
...
- [x] **Decimal arithmetic** — all prices and quantities are now `decimal.Decimal`,
      quantized to instrument precision using Saxo's `DisplayAndFormat.Decimals` field.
      Eliminates IEEE 754 rounding errors from the price and P&L display layer.
...
`claude plugin update saxotrader` triggers a Claude Code bug (confirmed
...
**Workaround — use `reinstall.sh`:**

```bash
./reinstall.sh
```

This script clears all stale cache and marketplace entries before reinstalling.
A plain uninstall alone leaves stale files in `~/.claude/plugins/marketplaces/`
that prevent the fresh version from loading — `reinstall.sh` handles all of that.
...
| [Claude Code — Plugin marketplaces](https://code.claude.com/docs/en/plugin-marketplaces) | Official docs for creating and distributing Claude Code plugin marketplaces. |
| [Claude Code — Plugins reference](https://code.claude.com/docs/en/plugins-reference) | Full plugin manifest schema (`plugin.json`) and CLI reference. |

## License

This project is released under the [MIT License](LICENSE).

Note that use of the Saxo OpenAPI is separately governed by Saxo Bank's
[3rd Party Non-Commercial License](skills/saxo/references/saxo-api-license.md),
which restricts usage to personal, non-commercial purposes on your own Saxo account.
See the live app disclaimer in [Register an app with Saxo](#1-register-an-app-with-saxo)
above for a summary of the key obligations.
